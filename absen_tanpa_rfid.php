...
// Menggunakan id_card sebagai identifikasi untuk memastikan data cocok antar database
// Logika: Cukup absen 1x (jam masuk), otomatis catat jam keluar jika sudah jam akhir
function recordAttendance($c, $id_karyawan_or_card, $clientTime = null, $id_card = null) {
    ...
    // Jika id_card disediakan, gunakan itu untuk mencari id_karyawan di database target
    $id = 0;
    if ($id_card) {
        $cardEsc = mysqli_real_escape_string($c, $id_card);
        $cardSql = "SELECT id_karyawan, jam_mulai, jam_akhir FROM karyawan WHERE id_card = '" . $cardEsc . "' LIMIT 1";
        $cardRes = @mysqli_query($c, $cardSql);
        if ($cardRes && mysqli_num_rows($cardRes) > 0) {
            $cardRow = mysqli_fetch_assoc($cardRes);
            $id = intval($cardRow['id_karyawan']);
            $jam_akhir = $cardRow['jam_akhir'] ?? null;
        }
    } else {
        $id = intval($id_karyawan_or_card);
        $jam_akhir = null;
    }
    
    if ($id === 0) {
        return ['success'=>false,'message'=>'ID karyawan tidak ditemukan di database absensi'];
    }
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        ...
        if (empty($row['jam_keluar'])) {
            // Cek apakah sudah jam akhir (otomatis catat jam keluar)
            if ($jam_akhir && strtotime($nowTime) >= strtotime($jam_akhir)) {
                // Sudah jam akhir atau lebih, otomatis catat jam keluar
                $upd = "UPDATE absensi SET jam_keluar = '" . $nowTime . "' WHERE id_absensi = " . intval($row['id_absensi']);
                try {
                    if (mysqli_query($c, $upd)) return ['success'=>true,'message'=>'Jam masuk: ' . $row['jam_masuk'] . ' | Jam keluar otomatis: ' . $nowTime];
                } catch (mysqli_sql_exception $e) {
                    return ['success'=>false,'message'=>'Gagal mencatat jam keluar: ' . $e->getMessage()];
                }
            } else {
                // Belum jam akhir, ingatkan pesan
                $sisaWaktu = '';
                if ($jam_akhir) {
                    $endTime = strtotime($jam_akhir);
                    $nowTimeStamp = strtotime($nowTime);
                    $diffMinutes = intval(($endTime - $nowTimeStamp) / 60);
                    if ($diffMinutes > 0) {
                        $sisaWaktu = " (Sisa waktu: " . $diffMinutes . " menit)";
                    }
                }
                return ['success'=>true,'message'=>'Jam masuk dicatat: ' . $nowTime . $sisaWaktu . '. Absen otomatis saat jam pulang tiba.'];
            }
        }
        return ['success'=>false,'message'=>'Absensi untuk hari ini sudah lengkap (masuk & keluar tercatat).'];
    } else {
        // insert jam_masuk saja
        $ins = "INSERT INTO absensi (id_karyawan, tanggal, jam_masuk) VALUES (" . $id . ", '" . $today . "', '" . $nowTime . "')";
        try {
            if (mysqli_query($c, $ins)) {
                // Cek jika sudah ada jam_akhir dan sudah lewat jam akhir
                if ($jam_akhir && strtotime($nowTime) >= strtotime($jam_akhir)) {
                    // Update langsung dengan jam keluar
                    $insId = mysqli_insert_id($c);
                    $upd = "UPDATE absensi SET jam_keluar = '" . $nowTime . "' WHERE id_absensi = " . $insId;
                    @mysqli_query($c, $upd);
                    return ['success'=>true,'message'=>'Jam masuk & keluar otomatis: ' . $nowTime];
                }
                return ['success'=>true,'message'=>'Jam masuk dicatat: ' . $nowTime . '. Absen otomatis saat jam pulang tiba.'];
            }
        } catch (mysqli_sql_exception $e) {
...
function getAllInternsAttendance($searchConns, $absConn) {
    ...
    if ($targetConn) {
        foreach ($interns as &$intern) {
            // Get attendance - gunakan id_card sebagai key, bukan id_karyawan
            $cardEsc = mysqli_real_escape_string($targetConn, $intern['id_card']);
            $sql = "SELECT a.jam_masuk, a.jam_keluar FROM absensi a
                   INNER JOIN karyawan k ON a.id_karyawan = k.id_karyawan 
                   WHERE k.id_card = '" . $cardEsc . "' 
            ...
            // Get izin if exists - gunakan id_card sebagai key
            $cardEsc2 = mysqli_real_escape_string($targetConn, $intern['id_card']);
            $sql = "SELECT i.jenis, i.status, i.alasan, i.created_at FROM izin i
                   INNER JOIN karyawan k ON i.id_karyawan = k.id_karyawan 
                   WHERE k.id_card = '" . $cardEsc2 . "' 
                   AND DATE(i.created_at) = '" . $today . "'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($selectedId > 0) {
        ...
        if (!$found) {
            ...
            $res = recordAttendance($targetConn, $found['id_karyawan'], $clientTime, $found['id_card']);
            $message = ['type'=> $res['success'] ? 'success' : 'error', 'text'=> $res['message']];
            // fetch recent attendance for display - gunakan id_card untuk fetch yang tepat
            $cardEsc = mysqli_real_escape_string($targetConn, $found['id_card']);
            $q2 = "SELECT a.tanggal, a.jam_masuk, a.jam_keluar FROM absensi a 
                   INNER JOIN karyawan k ON a.id_karyawan = k.id_karyawan 
                   WHERE k.id_card = '" . $cardEsc . "' ORDER BY a.tanggal DESC LIMIT 7";
        ...
        if ($input === '') {
            ...
            if (count($results) === 1) {
                ...
                $targetConn = $absConn ?: ($found['_db_handle'] ?: $defaultConn);
                $res = recordAttendance($targetConn, $found['id_karyawan'], null, $found['id_card']);
                $message = ['type'=> $res['success'] ? 'success' : 'error', 'text'=> $res['message']];
                // fetch recent attendance - gunakan id_card untuk fetch yang tepat
                $cardEsc = mysqli_real_escape_string($targetConn, $found['id_card']);
                $q2 = "SELECT a.tanggal, a.jam_masuk, a.jam_keluar FROM absensi a 
                       INNER JOIN karyawan k ON a.id_karyawan = k.id_karyawan 
                       WHERE k.id_card = '" . $cardEsc . "' ORDER BY a.tanggal DESC LIMIT 7";
...
<body>
    ...
    <footer style="text-align:center;padding:20px;color:#666;font-size:12px;margin-top:40px;border-top:1px solid #eef2f6;max-width:1200px;margin-left:auto;margin-right:auto">
