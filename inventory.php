...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_item') {
    ...
    if ($nama_barang !== '') {
        ...
        // Logika kode barang:
        // - Jika SN ada: gunakan SN atau kode yang diberikan (tidak auto-generate)
        // - Jika SN kosong: auto-generate kode barang
        ...
        if (empty($serial_number)) {
            // SN kosong: auto-generate kode barang
            if (empty($kode_barang)) {
                // Use MAX(kode_auto_number) to get next sequential number
                ...
                $kode_barang = '#' . str_pad($nextNum, 5, '0', STR_PAD_LEFT);  // #00001, #00002, etc.
            } else {
                // User provide code, ensure it starts with #
                if (strpos($kode_barang, '#') !== 0) {
                    $kode_barang = '#' . $kode_barang;
                }
            }
        } else {
            // SN ada: gunakan SN atau custom code (tidak auto-generate)
            if (empty($kode_barang)) {
                // Jika user tidak berikan kode, gunakan SN sebagai kode
                $kode_barang = $serial_number;
            }
        }
